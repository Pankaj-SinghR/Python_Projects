...
if response.status_code == 200:
    ...
    raw_data = soup.find_all("a", {"class": "Link--secondary no-underline no-wrap"})
    followers = raw_data[0].text.strip().split()[0]
    following = raw_data[1].text.strip().split()[0]
    
    additional_data = soup.find_all('a', {"class" : "Link--primary", "rel" : "nofollow me"})    
...
:::: Additional Information And Links ::::"""
    ...
    new_data = soup.find_all('div', {'class' : 'p-note user-profile-bio mb-3 js-user-profile-bio f4'})[0].find_all('a', {'class': 'user-mention'})
    for info in new_data:
        print(info.text)
    data = soup.find_all('div', {'class' : 'p-note user-profile-bio mb-3 js-user-profile-bio f4'})
    print(data[0].text)
